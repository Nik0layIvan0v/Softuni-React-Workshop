(function (global, factory) {
	...
	function createHandler(plugins, services) {
		return async function handler(req, res) {
			...
			async function handle(context) {
				const { serviceName, tokens, query, body } =
	...
	function parseWhere(query) {
		...
		try {
			...
			return (record) =>
				clauses.map((c) => c(record)).reduce(check, acc);
		} catch (err) {
			throw new Error(
				'Could not parse WHERE clause, check your syntax.'
			);
		}
	...
	function get(context, tokens, query, body) {
		...
		try {
			...
			if (query.offset) {
				responseData = responseData.slice(
			...
			if (query.load) {
				...
				props.map((prop) => {
					...
					const [idSource, collection] =
						relationTokens.split(':');
					console.log(
					...
					function transform(r) {
						...
						const related = storageSource.get(
							collection,
							seekId
						);
	...
	function post(context, tokens, query, body) {
		...
		try {
			responseData = context.storage.add(
				context.params.collection,
				body
			);
		} catch (err) {
	...
	function createInstance(seedData = {}) {
		...
		function query(collection, query) {
			...
			for (let [key, entry] of [...targetCollection.entries()]) {
				...
				if (match) {
					result.push(
						Object.assign(deepCopy(entry), { _id: key })
					);
				}
			}
			...
			if (userToken !== undefined) {
				...
				if (session !== undefined) {
					...
					if (userData !== undefined) {
						console.log(
							'Authorized as ' + userData[identity]
						);
						user = userData;
					}
				}
